...
export async function mePage(
  ...
  return page({
    ...
    <section class="grid">${mine.map((style) => `<article class="card"><h2>${escapeHtml(style.name)}</h2><p><span class="badge">${escapeHtml(statusLabel(style.status, locale))}</span></p><p class="muted">${escapeHtml(style.slug)}</p></article>`).join("")}</section>
